...
### changed
- the `cluster` block (bot filtering) now reaches the server/GitHub-App path, declared per repo in that repo's `config.json` alongside `incidents`. previously a repo-specific `cluster.bot_authors` was honoured by the CLI and silently ignored by the App, so the same repo produced different clusters depending on which path you looked at. closes #27
